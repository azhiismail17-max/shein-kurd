...
import { e as getLinkedOrdersInfo, g as getBoxName, f as fetchWithRetry, c as cleanOrderNo, h as isOrderFree, b as getDisplayPrice } from "./iraqi-BGJ5eM-r.mjs";
import OrderFormView from "./OrderFormView-Bb7tK8fX.mjs";
...
const OrderDetailModal = ({ order, onClose, onEdit, onDelete, allOrders, onSuccess, onStatusChange, onMissingClick, onUpdateOrder }) => {
  ...
  const handleWarningImageChange = async (e) => {
    ...
    try {
      const { uploadToImgBB } = await import("./order-utils-DvQyeH_v.mjs");
  ...
  const handlePdf = async () => {
    ...
    try {
      const { generateSingleOrderPdf } = await import("./pdf-D_sfiTgU.mjs");
