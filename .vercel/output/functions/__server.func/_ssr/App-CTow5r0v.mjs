...
import { a as fetchWithRetry, s as sendNotification, f as fetchNotifications, b as markNotificationRead, m as markNotificationFixed } from "./notifications-BuSzwt0M.mjs";
...
const DashboardView = React__default.lazy(() => import("./DashboardView-2PfAM4Xq.mjs"));
const OrderFormView = React__default.lazy(() => import("./OrderFormView-B_50ElEu.mjs"));
const OrderListView = React__default.lazy(() => import("./OrderListView-DuVsvVXE.mjs"));
const OrderDetailModal = React__default.lazy(() => import("./OrderDetailModal-j_A3KulD.mjs"));
const BatchesView = React__default.lazy(() => import("./BatchesView-CpIB6W2U.mjs"));
...
const MessagesView = React__default.lazy(() => import("./MessagesView-B3OSnqPn.mjs"));
const TeamView = React__default.lazy(() => import("./TeamView-BeC0wuCT.mjs"));
...
const Index = () => {
  ...
  const mergeOrders = reactExports.useCallback((prev, newData, sheetsToReplace) => {
  ...
  const applyApiResponse = reactExports.useCallback((json, isBackground) => {
    ...
    const sheets = data.length ? [...new Set(data.map((o) => o.sheet_name).filter(Boolean))] : [scriptActiveSheet];
  ...
  const silentRefresh = reactExports.useCallback((month) => {
    ...
    refreshTimeoutRef.current[target] = setTimeout(async () => {
      ...
      try {
        ...
        timeout = window.setTimeout(() => ctrl.abort(), 3e4);
  ...
  const handleStatusChange = reactExports.useCallback(async (order, newStatus) => {
    ...
    try {
      ...
      const { price: _price, sheet_price: _sheetPrice, customer_price: _customerPrice, ...orderWithoutPrice } = order;
