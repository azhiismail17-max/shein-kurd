...
import { Order, SCRIPT_URL, getOrderStatus, STATUS_COLORS, getVerifiedSet, saveVerifiedSet, getMissingSet, saveMissingSet, getMissingImages, saveMissingImages } from '@/types';
...
import { getLinkedOrdersInfo, getDisplayPrice, isOrderFree, cleanOrderNo, getBoxName } from '@/lib/order-utils';
              ...
              <div ref={receiptRef} className="bg-card border border-border rounded-2xl overflow-hidden">
                ...
                <div className="p-5 space-y-0.5">
                  ...
                  {linkedOrders.length === 0 ? (
                    <DetailRow 
                      label={<span>Item Price {isOrderFree(order) && <span className="text-[10px] bg-green-500/20 text-green-600 px-1 rounded uppercase font-bold ml-1">Free Ship</span>}</span>} 
                    ...
                    <>
                      ...
                      {linkedOrders.map(lo => (
                        <DetailRow 
                          ...
                          label={<span>🔗 {lo.name || lo.insta} {isOrderFree(lo) && <span className="text-[10px] bg-green-500/20 text-green-600 px-1 rounded uppercase font-bold ml-1">Free Ship</span>}</span>} 
                  ...
                  <DetailRow label="Shipping" value={isFree ? 'Free Ship (0 IQD)' : `${shipping.toLocaleString()} IQD`} />
                ...
                <div className="bg-secondary/50 p-3 rounded-lg">
                  ...
                  {linkedOrders.map(lo => (
                    <div key={`${lo.id}-${lo.sheet_name}`} className="flex justify-between items-center py-1.5 text-xs group transition-all">
                      <span>{lo.name || lo.insta} · #{lo.id} · {lo.sheet_name} {isOrderFree(lo) && <span className="text-[10px] bg-green-500/20 text-green-600 px-1 rounded uppercase font-bold ml-1">Free Ship</span>}</span>
                      <div className="flex items-center gap-2">
