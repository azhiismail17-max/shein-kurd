...
export default function ExpensesView() {
  ...
  const [webhookUrl, setWebhookUrl] = useState("https://script.google.com/macros/s/AKfycbxH9QSkC7roqWQ-rkKXvdqXVeDywrMJYDzdvJtbYNAD_sNJykxSpCt2JiEyJ267rlyv/exec");
  ...
  useEffect(() => {
    if (history.length > 0) {
      localStorage.setItem("expense-history", JSON.stringify(history.slice(0, 50))); // Keep last 50
    }
  }, [history]);

  return (
    <div className="h-full min-h-[calc(100vh-120px)] flex flex-col max-w-md mx-auto bg-[#FAF9F6] text-[#3D3D3D] font-sans sm:shadow-sm sm:border-x border-[#EFEBE0] rounded-2xl overflow-hidden mt-4">
      <Toaster position="top-center" />
      
      {/* Header */}
      <header className="px-6 py-6 flex items-center justify-between z-10 sticky top-0 bg-[#FAF9F6] sm:bg-transparent border-b border-[#EFEBE0]">
        <div className="flex items-center space-x-3">
          <div className="bg-[#8FA998] w-10 h-10 flex items-center justify-center rounded-xl text-white shadow-sm">
            <Banknote size={24} />
          </div>
          <div className="flex flex-col">
            ...
            <p className="text-xs text-[#8FA998] font-medium hidden sm:block">Connected to Google Sheets</p>
          </div>
        </div>
        <button 
...
function ExpenseForm({ 
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-8 bg-white rounded-[32px] shadow-sm border border-[#EFEBE0] p-8 mt-2">
      {/* Amount Input */}
      <div>
        <label className="block text-xs font-bold text-[#A5A58D] uppercase tracking-widest mb-3">Transaction Amount</label>
        <div className="relative">
          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-2xl font-light text-[#A5A58D]">IQD</span>
          <input
            type="number"
            step="1"
            inputMode="decimal"
      ...
      <button
        ...
        className={cn(
          ...
          isSubmitting || !amount
            ? "bg-[#A5A58D] cursor-not-allowed opacity-70"
            : "bg-[#8FA998] hover:bg-[#7D9686] shadow-lg shadow-[#8FA99833]"
        )}
      >
...
function RecentHistory({ history }: { history: ExpenseRecord[] }) {
  ...
  return (
    <div className="mt-8 flex-1 bg-white rounded-[32px] p-6 border border-[#EFEBE0] flex flex-col shadow-sm">
      <h3 className="text-sm font-bold text-[#A5A58D] uppercase tracking-widest mb-4">Last 5 Syncs (Local)</h3>
      <div className="space-y-2">
        {history.slice(0, 5).map(record => (
          <div key={record.id} className="flex items-center justify-between py-3 border-b border-[#F5F5F5] last:border-0">
            ...
            <span className="text-[#6B8E23] font-bold">{record.amount} IQD</span>
          </div>
        ))}
      </div>
    </div>
...
function SettingsPage({ 
  ...
  return (
    <div className="space-y-8 pb-12 bg-white rounded-[32px] shadow-sm border border-[#EFEBE0] p-8 mt-2">
      <div>
        <h2 className="text-xl font-bold tracking-tight text-[#2C3639] mb-2">Setup Guide</h2>
        <p className="text-[#A5A58D] text-sm leading-relaxed font-medium">
      ...
      <div className="space-y-6">
        {/* Step 1 */}
        <div className="flex bg-[#F8F9F3] p-5 rounded-2xl border border-[#E9EED9]">
          <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-[#8FA998] text-white font-bold mr-4 shadow-sm">
            1
          </div>
          <div>
            <h3 className="font-bold text-[#3D3D3D] mb-1">Create a Sheet</h3>
            <p className="text-sm text-[#7B8B6F]">Go to your Google Sheet. Identify the column you want to fill this month (e.g., column H).</p>
          </div>
        </div>
        ...
        <div className="flex bg-[#F8F9F3] p-5 rounded-2xl border border-[#E9EED9]">
          ...
          <div className="w-full">
            ...
            <p className="text-sm text-[#7B8B6F] mb-3">
              In your sheet menu, click <b>Extensions &gt; Apps Script</b>. Delete any existing code and paste this:
            </p>
            <div className="relative group">
              <pre className="bg-[#2C3639] text-[#EFEBE0] p-4 rounded-xl text-xs overflow-x-auto shadow-inner">
              ...
              <button 
                ...
                title="Copy script"
              >
                {copied ? <CheckCircle2 size={16} className="text-[#8FA998]" /> : <Copy size={16} />}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex bg-[#F8F9F3] p-5 rounded-2xl border border-[#E9EED9]">
          ...
          <div>
            ...
            <p className="text-sm text-[#7B8B6F]">
            ...
            <div className="mt-3 flex items-start space-x-2 text-[#7B8B6F] bg-white p-3 rounded-xl border border-[#E9EED9] text-sm shadow-sm">
              <AlertCircle size={16} className="mt-0.5 flex-shrink-0 text-[#A5A58D]" />
        ...
        {/* Step 4 */}
        <form onSubmit={handleSave} className="flex flex-col bg-white p-5 rounded-2xl border-2 border-[#8FA998] shadow-sm">
