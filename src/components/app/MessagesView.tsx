...
const TOPICS_COLLECTION = "kurdistani_topics";
const SYSTEM_LABEL = "Kurdistani";
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (file.type.startsWith("image/")) {
        ...
        const storageRef = ref(storage, `kurdistani-messages/${Date.now()}_${file.name}`);
    ...
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          ...
          const storageRef = ref(storage, `kurdistani-messages/${fileName}`);
      ...
      >
        <div className="border-b p-3 sm:p-4">
          <h2 className="flex items-center gap-2 text-base font-semibold sm:text-lg">
            <MessageCircle className="h-5 w-5" /> Kurdistani Messages
          </h2>
        </div>
