...
const OrderDetailModal: React.FC<Props> = ({
  ...
  const viewerRole =
    typeof window !== "undefined" ? localStorage.getItem("iraqi_auth_role") || "" : "";
  ...
  const handleRemoveLink = async (targetIo: Order) => {
    ...
    try {
      ...
      const remainingIds = remainingOrders.map((o) => `${o.id}:${o.sheet_name}`);
      ...
      updates.push(
        fetchWithRetry(SCRIPT_URL, {
          ...
          body: JSON.stringify({
            ...
            row_ids: [`${targetIo.id}:${targetIo.sheet_name}`],
          }),
        }),
      );
      ...
      if (onUpdateOrder) {
        onUpdateOrder(targetIo.id, targetIo.sheet_name, { linkedOrderIds: [] });
        remainingOrders.forEach((ro) => {
          onUpdateOrder(ro.id, ro.sheet_name, {
            linkedOrderIds: remainingOrders
                    ...
                    <DetailRow
                      label={
                        <span>
                          Item Price{" "}
                          {isOrderFree(order) && (
                            <span className="text-[10px] bg-red-900/20 text-red-900 px-1 rounded uppercase font-bold ml-1">
                              Free Ship
                            </span>
                          )}
                        </span>
