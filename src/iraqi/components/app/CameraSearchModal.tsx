      ...
      <div className="relative z-10 bg-card w-full h-full flex flex-col max-w-4xl mx-auto lg:my-4 lg:rounded-2xl lg:max-h-[calc(100vh-2rem)] lg:border lg:border-border lg:shadow-2xl overflow-hidden">
        ...
        <div className="p-4 border-b border-border flex items-center justify-between bg-secondary/50 shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="p-1.5 bg-secondary hover:bg-secondary/80 rounded-lg text-muted-foreground transition-colors">
              <ArrowLeft size={18} />
            </button>
            <div>
              <h2 className="font-bold text-lg">📷 Image Search</h2>
              <p className="text-muted-foreground text-xs">Find orders by comparing proof & warning images</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 bg-secondary hover:bg-secondary/80 rounded-lg text-muted-foreground transition-colors">
            <X size={18} />
        ...
        <div className="p-4 border-b border-border bg-card shrink-0 space-y-3">
          <div className="flex gap-2">
            <button onClick={() => cameraRef.current?.click()} className="flex-1 flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-3 rounded-xl transition-colors text-sm">
              <Camera size={18} /> Take Photo
            </button>
            <button onClick={() => fileRef.current?.click()} className="flex-1 flex items-center justify-center gap-2 bg-secondary hover:bg-secondary/80 text-foreground font-semibold py-3 rounded-xl transition-colors text-sm border border-border">
          ...
          {capturedImage && (
            <div className="relative">
              <img src={capturedImage} alt="Captured" className="w-full max-h-48 object-contain rounded-xl border border-border bg-secondary" />
              <button onClick={() => { setCapturedImage(null); setTargetHash(null); setFilterText(''); }} className="absolute top-2 right-2 p-1 bg-foreground/70 text-background rounded-full hover:bg-foreground transition-colors">
                <X size={14} />
              </button>
              {isComparing ? (
                <p className="text-[10px] text-primary flex items-center justify-center gap-1 mt-1 font-medium"><Loader2 size={12} className="animate-spin" /> Scanning images for match...</p>
              ) : (
                <p className="text-[10px] text-muted-foreground mt-1 text-center">Showing best matches below</p>
              )}
            </div>
          )}

          <div className="relative border-t border-border pt-3">
            <Search className="absolute left-3 top-[calc(50%+6px)] -translate-y-1/2 text-muted-foreground" size={16} />
            <input
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          <div className="flex justify-between items-end mb-3">
            <p className="text-xs text-muted-foreground font-medium">{displayedOrders.length} {targetHash && !isComparing ? "matching " : ""}orders found</p>
            {isComparing && <span className="text-xs text-primary flex items-center gap-1 font-medium"><Loader2 size={12} className="animate-spin" /> Processing</span>}
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {displayedOrders.map(order => {
              ...
              return (
                <div key={key} onClick={() => onOrderClick(order)} className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/30 hover:shadow-lg transition-all cursor-pointer group">
                  <div className="grid grid-cols-1 gap-0.5">
                    {proofSrc && (
                      <div className="aspect-square bg-secondary flex items-center justify-center overflow-hidden relative">
                        <img src={proofSrc} alt="Proof" referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        <span className="absolute top-1 left-1 bg-primary/80 text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded">PROOF</span>
                      </div>
                    )}
                    {primarySrc && (
                      <div className="aspect-square bg-secondary flex items-center justify-center overflow-hidden relative">
                        <img src={primarySrc} alt="Primary" referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        <span className="absolute top-1 left-1 bg-primary/80 text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded">PRIMARY</span>
                      </div>
                    )}
                    {warningSrc && (
                      <div className="aspect-square bg-secondary flex items-center justify-center overflow-hidden relative">
                        <img src={warningSrc} alt="Warning" referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        <span className="absolute top-1 left-1 bg-amber-500/80 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">WARNING</span>
                      </div>
                    )}
                    {order.secondaryImages?.map((img, i) => (
                      <div key={i} className="aspect-square bg-secondary flex items-center justify-center overflow-hidden relative">
                        <img src={img.startsWith('data:') ? img : `data:image/jpeg;base64,${img}`} alt={`Proof ${i + 2}`} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        <span className="absolute top-1 left-1 bg-primary/60 text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded">+{i + 1}</span>
                      </div>
                    ))}
                    {!proofSrc && !primarySrc && !warningSrc && !order.secondaryImages?.length && (
                  ...
                  <div className="p-2.5">
                    <div className="flex justify-between items-start gap-1 mb-0.5">
                      <span className="font-semibold text-xs truncate flex-1">{order.name || order.insta || '-'}</span>
                      <span className="text-primary font-bold text-xs whitespace-nowrap">{Number(String(order.price).replace(/[^0-9.-]+/g, '')).toLocaleString()}</span>
                    </div>
                    <div className="text-[10px] text-muted-foreground flex justify-between">
                      <span>#{order.id} · {order.sheet_name}</span>
            ...
            <div className="py-16 text-center">
              {isComparing ? (
                ...
                <>
                  ...
                  <p className="text-muted-foreground text-sm">{targetHash ? "No exact matches found" : "No orders with images found"}</p>
                </>
              )}
            </div>
          )}
