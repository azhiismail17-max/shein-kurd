...
import { MonthlyStats, YEARS_CONFIG } from "@/iraqi/types";
...
const CalculatorModal: React.FC<Props> = ({ monthlyStats, onClose }) => {
  ...
  const selectAll = () =>
    setSelected(Object.keys(monthlyStats).filter((m) => monthlyStats[m].count > 0));
  ...
  return (
    <div className="fixed inset-0 bg-foreground/30 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-card border border-border rounded-2xl w-full max-w-sm p-5 shadow-2xl animate-slide-up">
        ...
        <div className="max-h-48 overflow-y-auto mb-5 space-y-4 custom-scrollbar">
          {Object.entries(YEARS_CONFIG).map(([year, months]) => {
            const active = months.filter((m) => monthlyStats[m]?.count > 0);
            if (!active.length) return null;
            return (
              <div key={year}>
                <div className="text-[10px] font-semibold text-primary uppercase tracking-widest mb-2 border-b border-border pb-1">
                  {year}
                </div>
                <div className="grid grid-cols-3 gap-1.5">
                  {active.map((m) => (
                    <button
                      key={m}
                      onClick={() => toggle(m)}
                      className={`py-1.5 px-2 rounded-lg text-xs font-semibold border transition-all ${selected.includes(m) ? "bg-primary border-primary text-primary-foreground" : "bg-secondary border-border text-muted-foreground"}`}
                    >
                      {m}
                    </button>
                  ))}
        ...
        <div className="bg-secondary p-4 rounded-xl space-y-3 mb-5">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-card p-3 rounded-lg border border-border">
              ...
              <div className="text-lg font-bold">{totals.count.toLocaleString()}</div>
            </div>
            <div className="bg-card p-3 rounded-lg border border-border">
              <div className="text-[10px] text-muted-foreground font-medium mb-0.5">Revenue</div>
              <div className="text-lg font-bold">{totals.rev.toLocaleString()}</div>
            </div>
          </div>
          ...
          <div className="bg-card p-3 rounded-lg border border-border space-y-2">
            <div className="flex justify-between text-xs">
              ...
              <span className="text-destructive font-mono font-semibold">
                {totals.buy.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Weight/Ship</span>
              <span className="text-destructive font-mono font-semibold">
                {totals.wgt.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Other</span>
              <span className="text-destructive font-mono font-semibold">
                {totals.etc.toLocaleString()}
              </span>
            </div>
            <div className="border-t border-border pt-2 flex justify-between text-xs font-bold">
              <span>Total Cost</span>
              <span className="font-mono">{totals.totalCost.toLocaleString()}</span>
            </div>
            {totals.lost > 0 && (
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">Lost (K3)</span>
                <span className="text-destructive font-mono font-semibold">
          ...
          <div className="text-center pt-2">
            ...
            <div
              className={`text-2xl font-bold font-mono ${totals.profit >= 0 ? "text-primary" : "text-destructive"}`}
            >
        ...
        <button
          onClick={onClose}
          className="w-full bg-foreground hover:bg-foreground/90 text-background font-semibold py-3 rounded-lg transition-all text-sm"
        >
          Close
        </button>
      </div>
