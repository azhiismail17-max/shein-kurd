...
import { SystemKey, SystemSwitcher } from '@/components/app/SystemSwitcher';
...
const AppLayout: React.FC<AppLayoutProps> = ({
  ...
  currentSystem = 'iraqi', onSystemChange
}) => {
        ...
        <header className="bg-card/80 backdrop-blur-xl border-b border-border sticky top-0 z-30 shrink-0">
          <div className="px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              {role === 'owner' && (
                <button onClick={onSearchAll} className="hidden sm:block p-2 text-muted-foreground hover:text-primary hover:bg-secondary rounded-lg transition-colors" title="Global Ledger">
                  <Calculator size={18} />
                </button>
              )}
            </div>
