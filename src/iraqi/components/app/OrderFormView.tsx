...
import { findAutoBoxName } from '@/iraqi/lib/autoBox';
import { getDisplayPrice, getSheetPriceForSave, isOrderFree, uploadToImgBB, fileToBase64 } from '@/iraqi/lib/order-utils';
...
const OrderFormView: React.FC<OrderFormViewProps> = ({ activeSheet, editingOrder, onCancel, onSuccess, allOrders }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('iraqi_last_region') as Region;
    if (saved && !isEditing) {
      setRegion(saved);
    }
  }, [isEditing]);
  ...
  const [activeSuggestionField, setActiveSuggestionField] = useState<'insta' | 'name' | 'phone'>('insta');
  ...
  const customerDb = useMemo(() => {
    const map = new Map<string, { insta: string; name: string; phone: string; phone2: string; place: string; pics_text: string }>();
    allOrders.forEach(o => {
  ...
  useEffect(() => {
    if (editingOrder) {
      ...
      if (editingOrder.proof_urls) {
        proofUrlArray = (Array.isArray(editingOrder.proof_urls)
      ...
      // Only load manual composite links (id:sheet), ignore auto-merge row numbers
  ...
  const searchCustomers = (value: string, field: 'insta' | 'name' | 'phone') => {
    ...
    if (val.length === 0) { setShowSuggestions(false); return; }
    ...
    setSuggestions(unique.map(c => ({ insta: c.insta, name: c.name, phone: c.phone, phone2: c.phone2, place: c.place, pics_text: c.pics_text } as Order)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = { 
          ...
          client_request_id: tempId ? `iraqi-${activeSheet}-${tempId}` : undefined
      };
      ...
      if (payload.pics_text || payload.link) {
        try {
          ...
          }).catch(() => { /* silently ignore secondary script errors */ });
        } catch (err) {
          // silently ignore
        }
      }
      ...
      (async () => {
        try {
          ...
          if (result?.status === 'success') {
            ...
            } catch (e) { console.error('Failed to update Iraqi user stats', e); }
  ...
  const renderField = (label: string, fieldName: string, type = 'text', placeholder = '', required = false) => (
    <div className="space-y-1.5">
      ...
      <input
        type={type} name={fieldName} value={String((formData as any)[fieldName] || '')}
        onChange={handleInputChange} placeholder={placeholder} required={required}
        onFocus={() => {
  ...
  const SuggestionsDropdown = () => {
    ...
    return (
      <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg overflow-hidden flex flex-col">
        <div className="flex justify-between items-center px-3 py-1.5 bg-secondary border-b border-border">
          <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Suggestions</span>
          <button type="button" onClick={() => setShowSuggestions(false)} className="p-1 hover:bg-muted text-muted-foreground rounded-full transition-colors"><X size={12} /></button>
        </div>
        <div className="max-h-48 overflow-y-auto">
          {suggestions.map((s, i) => (
            <button key={i} type="button" onClick={() => selectSuggestion(s)} className="w-full text-left px-3 py-2.5 hover:bg-secondary transition-colors text-sm border-b border-border/50 last:border-0">
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-md sm:max-w-lg mx-auto bg-card p-3 sm:p-6 rounded-lg sm:rounded-xl border border-border animate-slide-up">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onCancel} className="p-2 bg-secondary hover:bg-secondary/80 rounded-lg text-muted-foreground transition-colors">
            <X size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3 sm:space-y-4" ref={suggestionContainerRef}>
        ...
        <div className="grid grid-cols-1 min-[390px]:grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select name="region" value={region} onChange={handleInputChange} className={inputClass}>
        ...
        <div className="border border-border p-3 sm:p-4 rounded-xl bg-secondary/10 mt-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {[...existingPrimaryUrls, ...previewPrimaryUrls].map((img, i) => (
              <div key={`prim-${i}`} className="relative aspect-square rounded-lg overflow-hidden border border-border group cursor-pointer" onClick={() => setViewingImage(img)}>
                <img src={img} alt="Primary" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                <span className="absolute top-1 left-1 bg-primary text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded shadow z-10">PRIMARY</span>
                ...
                }} className="absolute top-1 right-1 p-1.5 bg-destructive text-destructive-foreground rounded-full shadow-md z-10 hover:bg-red-600 active:scale-95"><X size={14} /></button>
              </div>
            ))}
        ...
        <div className="border border-border p-3 sm:p-4 rounded-xl bg-secondary/10 mt-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {[...existingProofUrls, ...previewProofUrls].map((img, i) => (
              <div key={`proof-${i}`} className="relative aspect-square rounded-lg overflow-hidden border border-border group cursor-pointer" onClick={() => setViewingImage(img)}>
                <img src={img} alt="Proof" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                <span className="absolute top-1 left-1 bg-primary/80 text-primary-foreground text-[10px] font-bold px-1.5 py-0.5 rounded shadow z-10">PROOF</span>
            ...
            <div onClick={() => proofFileRef.current?.click()} className="relative aspect-square rounded-lg border-2 border-dashed border-primary/40 flex flex-col justify-center items-center cursor-pointer hover:border-primary transition-all group bg-primary/5">
               <Upload size={18} className="text-primary/70 group-hover:text-primary transition-colors mb-1" />
               <span className="text-[10px] font-bold text-primary/70 group-hover:text-primary leading-tight text-center px-1">Add Proof Image</span>
            </div>
          </div>
        </div>
        ...
        {region && (
          <div className="flex items-center justify-between bg-secondary/50 border border-border rounded-lg px-3 sm:px-4 py-3 gap-3">
            ...
            >
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${freeShipping ? 'translate-x-5' : ''}`} />
            </button>
          </div>
        )}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Linked Orders</label>
            <button type="button" onClick={() => setShowLinkModal(true)} className="flex items-center gap-1 text-xs text-primary font-medium hover:underline">
          ...
          {linkedOrders.length > 0 && (
            <div className="space-y-1">
              {linkedOrders.map(o => (
                <div key={`${o.id}-${o.sheet_name}`} className="flex items-center justify-between bg-secondary px-3 py-2 rounded-lg text-sm">
                  <span className="font-medium">{o.name || o.insta} <span className="text-muted-foreground text-xs">#{o.id} · {o.sheet_name}</span></span>
                  <button type="button" onClick={() => setLinkedIds(p => p.filter(id => id !== `${o.id}:${o.sheet_name}` && id !== o.id))} className="text-muted-foreground hover:text-destructive"><X size={14} /></button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-secondary/30 border border-primary/20 p-3 sm:p-4 rounded-xl space-y-2 mt-4">
          ...
          <div className="flex justify-between text-xs text-muted-foreground">
            ...
            <span>{(parseFloat(String(formData.price).replace(/[^0-9.-]+/g, '')) || 0).toLocaleString()} IQD</span>
          </div>
          {linkedOrders.length > 0 && (
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>Linked Items ({linkedOrders.length})</span>
              <span>{linkedOrders.reduce((sum, o) => sum + getDisplayPrice(o, allOrders), 0).toLocaleString()} IQD</span>
            </div>
          )}
          ...
          <div className="flex justify-between text-sm font-medium pt-1">
            ...
            {(() => {
              ...
              const isAutoFree = cartTotal >= 118000;
              ...
              if (isAutoFree || freeShipping) {
                return (
                  <span className="text-primary font-bold flex items-center gap-1">
                    0 IQD <span className="bg-primary/20 px-1.5 py-0.5 rounded text-[10px] uppercase">Free Delivery Activated</span>
                  </span>
                );
              }
      ...
      {viewingImage && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 p-4">
          <button onClick={() => setViewingImage(null)} className="absolute top-4 right-4 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors">
            <X size={24} />
          </button>
          <img src={viewingImage} alt="Expanded view" referrerPolicy="no-referrer" className="max-w-full max-h-full object-contain rounded-lg" />
        </div>
      )}
      ...
      {showLinkModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-foreground/30 backdrop-blur-sm" onClick={() => setShowLinkModal(false)} />
          <div className="bg-card border border-border w-full max-w-sm rounded-2xl shadow-2xl relative z-10 animate-slide-up">
            <div className="p-4 border-b border-border flex justify-between items-center">
              ...
              <button onClick={() => setShowLinkModal(false)} className="p-1.5 bg-secondary rounded-lg text-muted-foreground"><X size={18} /></button>
            </div>
            <div className="p-4 space-y-3">
              <input value={linkSearch} onChange={e => setLinkSearch(e.target.value)} placeholder="Search by name, insta, phone..." className="w-full bg-secondary border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary/50" />
              <div className="max-h-60 overflow-y-auto space-y-1 custom-scrollbar">
                {linkableOrders.map(o => (
                  <button key={`${o.id}-${o.sheet_name}`} type="button" onClick={() => { setLinkedIds(p => [...p, `${o.id}:${o.sheet_name}`]); setShowLinkModal(false); }} className="w-full flex justify-between items-center px-3 py-2 rounded-lg hover:bg-secondary transition-colors text-left text-sm">
                    <div>
                      <span className="font-medium">{o.name || o.insta}</span>
                      <span className="text-xs text-muted-foreground ml-2">#{o.id} · {o.sheet_name}</span>
                    </div>
                    <span className="text-xs text-muted-foreground font-mono">{Number(String(o.price).replace(/[^0-9.-]+/g, '')).toLocaleString()}</span>
                  </button>
                ))}
                {linkableOrders.length === 0 && <p className="text-center text-muted-foreground text-sm py-4">No orders found</p>}
              </div>
            </div>
          </div>
        </div>
