...
import { submitIraqiOrder } from "@/lib/submitOrder";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentUser =
        ...
        resolveTeamUsername("iraqi", currentRole) ||
        currentRole;
      ...
      const payload: any = {
        ...
        client_request_id: tempId ? `iraqi-${targetSheet}-${tempId}` : undefined,
      };
      if (isEditing) {
      ...
      (async () => {
        try {
          const result = await submitIraqiOrder(
            {
              name: formData.name,
          ...
          try {
            await recordOrderCreated("iraqi", currentUser, currentRole, {
              ...payload,
