...
function dedupeRecentOrders(orders: CombinedRecentOrder[]) {
  ...
  for (const order of orders) {
    ...
    if (
      !existing ||
      new Date(order.createdAt || 0).getTime() > new Date(existing.createdAt || 0).getTime()
    ) {
      byKey.set(key, order);
    }
  }
...
export function getUserStatsForProfile(
  stats: Record<string, any>,
  username: string,
  profile: ProfileSource | "all",
) {
  ...
  const bySource = sources.map((source) => {
    ...
    const recentOrders = dedupeRecentOrders(
      Array.isArray(sourceStats.recentOrders)
    ...
    return {
      ...
      todayOrders,
      isOnline:
        sourceStats.isOnline === true &&
        sourceStats.lastActive &&
        Date.now() - new Date(sourceStats.lastActive).getTime() < 90000,
      isActiveToday: sourceStats.lastActive
        ? new Date(sourceStats.lastActive).toDateString() === new Date().toDateString()
        : false,
      recentOrders,
    };
  });
...
export async function fetchCombinedAuthUsers(
  fetcher: typeof fetch,
  profile: ProfileSource | "all" = "all",
): Promise<CombinedAuthUser[]> {
  const urls =
    profile === "all"
      ? [KURDISTANI_SCRIPT_URL, IRAQI_SCRIPT_URL]
      : [profile === "iraqi" ? IRAQI_SCRIPT_URL : KURDISTANI_SCRIPT_URL];
  const responses = await Promise.allSettled(
    urls.map((url) => fetcher(`${url}?action=get_auth_users`)),
  );
  ...
  for (const response of responses) {
    ...
    for (const rawUser of data.users) {
      ...
      users.set(key, {
        ...
        role:
          existing?.role && existing.role !== existing.username ? existing.role : formatted.role,
      });
    }
  }
