...
export async function recordPresence(
  ...
  await setDoc(
    doc(db, "user_stats", getTeamStatsId(profile, cleanUsername)),
    {
...
export async function recordOrderCreated(
  ...
  const recentOrders =
    existing.exists() && Array.isArray((existing.data() as any).recentOrders)
      ? (existing.data() as any).recentOrders
      : [];
  const alreadyTracked = recentOrders.some(
    (item: any) =>
      String(item.id) === String(orderId) &&
      String(item.sheet || item.sheet_name || "") === String(sheet),
  );
...
export async function recordOrderDeleted(
  profile: TeamProfile,
  sheet: string,
  rowId: string | number,
) {
  ...
  const updates = snap.docs.map(async (docSnap) => {
    ...
    const removed = data.recentOrders.filter(
      (order: any) => String(order.id) === String(rowId) && String(order.sheet) === String(sheet),
    );
    if (removed.length === 0) return;

    const remaining = data.recentOrders.filter(
      (order: any) =>
        !(String(order.id) === String(rowId) && String(order.sheet) === String(sheet)),
    );
    const removedToday = removed.filter(
      (order: any) =>
        order.createdAt &&
